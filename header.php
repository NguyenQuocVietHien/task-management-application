<head>
  ...
  <meta charset="utf-8">
  <!-- Define the initial viewport settings for responsive design -->
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <?php 
  date_default_timezone_set("Asia/Ho_Chi_Minh"); // Set the default timezone
  // Start output buffering to capture and store the generated title
  ob_start();
  // Determine the page title based on the 'page' parameter from the URL
  $title = isset($_GET['page']) ? ucwords(str_replace("_", ' ', $_GET['page'])) : "Home";
  ?>
  <!-- Set the page title, dynamically generated based on the URL parameter -->
  <title><?php echo $title ?> | <?php echo $_SESSION['system']['name'] ?></title>
  <!-- End and flush the output buffer to display the generated title -->
  <?php ob_end_flush() ?> 
