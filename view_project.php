...
// Define an array to map project statuses to corresponding labels.
$stat = array("Pending","Started","On-Progress","On-Hold","Over Due","Done"); 
// Query the project details from the database.
$qry = $conn->query("SELECT * FROM project_list where id = ".$_GET['id'])->fetch_array(); 
foreach($qry as $k => $v){
	// Create variables for each project detail fetched from the database.
	$$k = $v; 
}
// Count the total number of tasks for this project.
$tprog = $conn->query("SELECT * FROM task_list where project_id = {$id}")->num_rows;
// Count the completed tasks for this project.
...
// Count the number of user productivities for this project.
...
if($prod  > 0  || $cprog > 0)
// Set the project status to 'On-Progress' if there is productivity data or completed tasks.
  $status = 2;
else
// Set the project status to 'Started' if no productivity data or completed tasks.
  $status = 1;
elseif($status == 0 && strtotime(date('Y-m-d')) > strtotime($end_date)):
// Set the project status to 'Over Due' if the project's end date has passed.
$status = 4;
endif;
// Query the project manager's details.
$manager = $conn->query("SELECT *,concat(firstname,' ',lastname) as name FROM users where id = $manager_id");
// Fetch the project manager's information or set to an empty array if not found.
					...
					<div class="row">
						...
						<div class="col-md-6">
							...
							<dl>
								...
								<dd>
									<?php
									// Display the project status with appropriate badge colors.
						...
						<tbody>
							...
							?>
								<tr>
			                        ...
			                        <td>
			                        	<?php 
										// Display the task status with appropriate badge colors.
			                        	if($row['status'] == 1){
